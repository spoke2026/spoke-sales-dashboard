...
describe('isUuid', () => {
  ...
  test('rejects non-strings and malformed text', () => {
    for (const v of [null, undefined, 1, {}, '', 'not-a-uuid', `${UUID_A}x`, ` ${UUID_A}`]) {
...
describe('parseTargetText', () => {
  test("'' and spaces are an empty cell", () => {
    expect(parseTargetText('count', '')).toEqual({ ok: true, value: null })
    expect(parseTargetText('count', '   ')).toEqual({ ok: true, value: null })
  })
  test("'0' is a value of 0, not empty (divergence: truthiness)", () => {
    expect(parseTargetText('count', '0')).toEqual({ ok: true, value: 0 })
    expect(parseTargetText('percent', '0')).toEqual({ ok: true, value: 0 })
  })
  test('trims, and converts percent to a fraction', () => {
    expect(parseTargetText('count', ' 20.5 ')).toEqual({ ok: true, value: 20.5 })
    expect(parseTargetText('percent', '33')).toEqual({ ok: true, value: 0.33 })
    expect(parseTargetText('currency', '1500')).toEqual({ ok: true, value: 1500 })
  })
  test('rejects negatives, commas, symbols, exponents and words', () => {
    for (const text of ['-1', '1,000', '$5', '5%', '1e3', 'abc', '.5', '5.', '2 0']) {
      expect(parseTargetText('count', text), text).toEqual({ ok: false })
    }
  })
  test('15 characters is accepted and 16 is rejected', () => {
    expect(parseTargetText('count', '123456789012345')).toEqual({ ok: true, value: 123456789012345 })
    expect(parseTargetText('count', '1234567890123456')).toEqual({ ok: false })
    expect(parseTargetText('count', '  123456789012345  ')).toEqual({ ok: true, value: 123456789012345 })
  })
})
