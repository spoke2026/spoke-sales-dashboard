...
export async function getConnectedCalls(monthStart, monthEnd, ownerIds = null) {
  const results = await fetchAll(
    `/crm/v4/objects/calls?properties=hs_call_status,hs_createdate,hubspot_owner_id`
  )
  const calls = results.filter(c => {
    const d = new Date(c.properties.hs_createdate)
