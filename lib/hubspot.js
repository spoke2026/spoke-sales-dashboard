...
// HubSpot returns the meeting outcome as an uppercase enum (COMPLETED, NO_SHOW).
// Render it as readable text; fall back to a neutral label when unset.
function meetingOutcome(value) {
  if (!value) return 'Visit'
  const words = String(value).toLowerCase().replace(/_/g, ' ')
  return words.charAt(0).toUpperCase() + words.slice(1)
}

// Association payload shape differs between API versions and endpoints: some
// return { id }, others { toObjectId }. Read whichever is present.
function associatedIds(record, objectType) {
  const results = record.associations?.[objectType]?.results || []
  return results.map(r => String(r.toObjectId ?? r.id)).filter(Boolean)
}

// Resolve a set of object IDs to properties via the batch read endpoint.
// Chunked at 100 (the HubSpot batch limit). Returns a Map of id -> properties.
// Fails soft: a lookup problem must not take down the whole dashboard, so on
// error this returns what it has and the caller degrades to a generic label.
async function batchReadProperties(objectType, ids, properties) {
  ...
  for (let i = 0; i < unique.length; i += 100) {
    ...
    try {
      ...
      ;(data.results || []).forEach(r => byId.set(String(r.id), r.properties || {}))
    } catch (e) {
      console.error(`HubSpot batch read failed for ${objectType}:`, e.message)
    }
  }
  return byId
}
...
export async function getMeetings(monthStart, monthEnd, ownerIds = null) {
  // associations pull the company/contact each meeting was with — the meeting
  // object itself carries no company name.
  const results = await fetchAll(
    `/crm/v4/objects/meetings?properties=hs_meeting_start_time,hs_createdate,hubspot_owner_id,hs_meeting_title,hs_meeting_outcome&associations=companies,contacts`
  )
  ...
  // Resolve who each visit was with. Only the in-month, in-scope meetings are
  // looked up, so this is two extra calls regardless of how many meetings the
  // portal holds overall.
  const companyIds = meetings.flatMap(m => associatedIds(m, 'companies'))
  const contactIds = meetings.flatMap(m => associatedIds(m, 'contacts'))

  const [companiesById, contactsById] = await Promise.all([
    batchReadProperties('companies', companyIds, ['name']),
    batchReadProperties('contacts', contactIds, ['firstname', 'lastname', 'company']),
  ])

  // Prefer the associated company. Where a meeting is only linked to a contact,
  // use that contact's company, then their name. The meeting's own title is a
  // late fallback because HubSpot commonly leaves it blank.
  function visitTitle(m) {
    for (const id of associatedIds(m, 'companies')) {
      const name = companiesById.get(id)?.name
      if (name) return name
    }
    for (const id of associatedIds(m, 'contacts')) {
    ...
    if (title) return title.replace('Meeting with ', '')
    return 'Face to face visit'
  }

  // Same shape as the calls drill-down: title, owner, date, most recent first.
  // Date is the meeting start (falling back to created), matching the filter above.
  const details = meetings.map(m => ({
    title: visitTitle(m),
    outcome: meetingOutcome(m.properties.hs_meeting_outcome),
    owner: m.properties.hubspot_owner_id,
    date: m.properties.hs_meeting_start_time || m.properties.hs_createdate,
  })).sort((a, b) => new Date(b.date) - new Date(a.date))

  return { total: meetings.length, dailyActuals: cumDaily, byOwner, details }
}
...
export async function getDealAge(monthStart, monthEnd, ownerIds = null) {
  ...
  const DONE_STAGES = [
  '2795435483', // Goods Shipped
  '2795858407', // Invoiced
  '2795858408', // Closed Lost
  '2795858402', // Samples Sent
]
  ...
  do {
    const body = {
      filterGroups: [{
        filters: [
  { propertyName: 'pipeline', operator: 'EQ', value: pipeline },
  { propertyName: 'closedate', operator: 'GTE', value: String(new Date(monthStart.getFullYear(), monthStart.getMonth() - 1, 1).getTime()) },
  { propertyName: 'closedate', operator: 'LTE', value: String(monthEnd.getTime()) },
]
      }],
