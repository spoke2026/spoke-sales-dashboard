...
export async function getDealAge(monthStart, monthEnd, ownerIds = null) {
  ...
  // Use search to filter by pipeline and close date server-side
  let allDeals = []
  let after = null
  do {
    const body = {
  ...
  // Filter to active deals only — not yet at Goods Shipped, Invoiced or Closed Lost
  ...
  const bands = BAND_DEFS.map(b => ({
    ...
    deals: activeDeals
      ...
      .map((d, i, arr) => {
        const idx = activeDeals.indexOf(d)
        return {
          name: d.properties.dealname || 'Unnamed deal',
          days: ages[idx],
          stage: d.properties.dealstage,
  ...
  return { avgDays: avg, total: activeDeals.length, bands, history: [avg] }
}